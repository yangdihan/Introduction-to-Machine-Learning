...
def preprocess_data(dataset,
    ...
    for data in dataset:
        ...
        for feature in feature_columns:
            idx = columns_to_id[feature]
            if (idx == 1):
                row.extend(one_hot_bldg_type(props[1]))
            else:
                if (squared_features):
                    row.append( (int(props[idx]))**2 )
                else:
                    row.append(int(props[idx]))

        x.append(row)
        y.append(props[5])

    x = np.array(x).astype(int)
    y = np.array(y).reshape((N,1)).astype(int)
