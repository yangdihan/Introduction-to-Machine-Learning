...
class LinearMRF(object):
    ...
    def get_pairwise_features(self):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        cell = np.array([1,0,0,1])
        result = np.tile(cell,(len(self.pairs),1))
        return result
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        # feature = tf.convert_to_tensor(unary_features,dtype=tf.float32)
        # result = tf.multiply(self.unary_weight,feature)
        # return result
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        # feature = tf.convert_to_tensor(pairwise_features,dtype=tf.float32)
        # result = tf.multiply(self.pairwise_weight,feature)
        # return result
...
.astype
        ...
        #######################
        obj = 0
        num_noise = len(unary_beliefs)
        # true_features = tf.convert_to_tensor(img_features,dtype=tf.int32)
        true_features = tf.convert_to_tensor(img_features)
        true_features = tf.cast(true_features,unary_potentials[0].dtype)
        for n in range(num_noise):
            obj += tf.reduce_sum(tf.multiply(unary_beliefs[n],unary_potentials[n]))
            obj += tf.reduce_sum(tf.multiply(pair_beliefs[n],pairwise_potentials[n]))
            # obj += tf.reduce_sum((unary_beliefs[n]*unary_potentials[n]))
            # obj += tf.reduce_sum((pair_beliefs[n]*pairwise_potentials))
            # obj -= tf.reduce_sum(true_features*unary_potentials)
            obj -= tf.reduce_sum(img_features*unary_potentials[n])
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        nodes = np.arange(self.width*self.height)
        unary_beliefs = unary_beliefs.copy()
        np.random.shuffle(nodes)
        for node in nodes:
            score0 = self.calculate_local_score(node, 0, unary_beliefs, unary_pots, pairwise_pots)
            score1 = self.calculate_local_score(node, 1, unary_beliefs, unary_pots, pairwise_pots)
            if (score0 > score1):
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        local_potential = unary_potentials[node,assignment]
        score += local_potential

        # local_belief = unary_beliefs[node,assignment]
        local_belief = assignment
        for neighbor in self.neighbors[node]:
            # neighbor_belief = unary_beliefs[neighbor,1]
            neighbor_belief = np.where(unary_beliefs[neighbor]==1)[0][0]
            pair = self.pair_inds[(neighbor,node)]
            neighbor_potential = pairwise_potentials[pair]
            if (local_belief==0 and neighbor_belief==0):
                score += neighbor_potential[0]
            elif (local_belief==0 and neighbor_belief==1):
                score += neighbor_potential[1]
            elif (local_belief==1 and neighbor_belief==0):
                score += neighbor_potential[2]
            elif (local_belief==1 and neighbor_belief==1):
                score += neighbor_potential[3]
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        change = np.sum(np.abs(new_unary_beliefs-old_unary_beliefs))/2
        change /= len(new_unary_beliefs)
        return (change<convergence_margin)
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        Specifically, given a pair of nodes (m, n), the pairwisve belief
        ...
        for pair in self.pairs:
            m = pair[0]
            n = pair[1]
            if (unary_beliefs[m,1]==0 and unary_beliefs[n,1]==0):
                result[self.pair_inds[pair],0]=1
            elif (unary_beliefs[m,1]==0 and unary_beliefs[n,1]==1):
                result[self.pair_inds[pair],1]=1
            elif (unary_beliefs[m,1]==1 and unary_beliefs[n,1]==0):
