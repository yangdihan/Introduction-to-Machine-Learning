...
def get_action_index(readout_t, epsilon, t):
    ...
    if (t<OBSERVE or np.random.random() < epsilon):
        action_index = np.random.randint(low=0, high=ACTIONS)
    else:
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    x_4, r, terminal = game_state.frame_step(a_t)
    x_4 = cv2.cvtColor(cv2.resize(x_4, (80, 80)), cv2.COLOR_BGR2GRAY)
    ret, x_4 = cv2.threshold(x_4, 1, 255, cv2.THRESH_BINARY)

    x_1 = s_t[:,:,1]
    x_2 = s_t[:,:,2]
    x_3 = s_t[:,:,3]

    s_t = np.stack((x_1, x_2, x_3, x_4), axis=2)
    return s_t, r, terminal
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if (terminal_batch[i]):
            ...
            Q = r_batch[i] + GAMMA*np.max(readout_j1_batch[i])
        target_q_batch.append(Q)
