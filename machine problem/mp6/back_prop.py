...
class Neural_Network():
    ...
    def forward(self, X):
        ...
        '''
        # Z = self.fully_connected(X, self.U, np.tile(self.e,(batchSize,1)))
    ...
    def error_at_layer1(self, delta_k, W, b):
        ...
        '''
        ## how to sum??
        # delta_j = np.multiply(delta_k,np.multiply(self.sigmoidPrime(b), np.sum(W,axis=1)))
        delta_j = np.multiply(self.sigmoidPrime(b),np.dot(delta_k, W.T))
        return delta_j
